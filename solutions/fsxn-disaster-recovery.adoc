...
* 你需要下载 https://console.netapp.com/automationHub["Amazon FSx for NetApp ONTAP管理 - 灾难恢复"^] 通过NetApp ConsoleWeb UI 实现自动化解决方案。该解决方案打包如下： `FSxN_DR.zip`。此压缩包包含 `AWS_FSxN_Bck_Prov.zip` 您将使用此文件来部署本文档中描述的解决方案。
* 源系统和目标系统之间的网络连接。
* 具有以下特征的Linux VM：
+
** 基于Debian的Linux分发版
